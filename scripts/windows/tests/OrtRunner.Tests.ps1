...
Describe 'WindowsOrtRunner.Common' {
    ...
    It 'refuses to stage without a chain ONNX_ROOT, and names the hub commit when G6 is missing' {
        $c = New-Case 'noroot'
        Get-ThrowText { Copy-RunnerChainOrt -OnnxRoot '' -RunnerDir $c.Runner } | Should Match 'ONNX_ROOT is unset'
